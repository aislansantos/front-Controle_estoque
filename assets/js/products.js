class Product {
  constructor(endpoint) {
    ...
    this.endpointProduct = this.endpoint;
  ...
  async loadProducts() {
    try {
      ...
      const response = await fetch(this.endpointProduct, {
        method: "GET",
        headers: {
  ...
  async loadProductDetails(productId, categories, units) {
    try {
      ...
      const response = await fetch(`${this.endpointProduct}${productId}`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
        },
